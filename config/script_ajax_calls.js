    ...
    try {
        ...
        if (filteredResults.length > 0) {
            await filteredResults.forEach(place => {
                ...
                ) { if ((parseInt(someCounter) % 6) == "0") { divNo++; }; someCounter++; results += `<div class="divResults" data-pageResults="${divNo}" onclick="openAddMarkModal2(${place.ID},${user})"><img src="${place.photo}" alt="photos"><h4 class="overflow-elipsis">${place.name}</h4><p class="overflow-elipsis">${place.type} in ${place.location}</p><p class="overflow-elipsis">${place.description}</p><p style='font-size:1.2rem;font-weight:bolder;white-space: nowrap;margin-left:20px;'>£${place.price}/night</p><button style="border:none;background-color:transparent" type="button" onclick="openAddMarkModal2(30,1)"><img src="./img/booknow.png" alt="Get Directions" id="directionIcon" style="width:45px; height:45px;background-color:transparent;"></button></div>` }
